...
def dfs_to_data(variable_df, extraction_df, encoder):
    def _get_response(variable_id: int, column: str):
        extractions = extraction_df[extraction_df.variable_id == variable_id]
        valid_extractions =
    X = extraction_df.pivot(
        index='variable_id', columns='method', values='confidence').reset_index(level=0)
    var_values = X.variable_id.apply(
        lambda x: _get_response(x, "variable_value"))
    X = pd.concat([X, pd.DataFrame(encoder.transform(
        var_values.values.reshape(-1, 1)).toarray(), dtype=int)], axis=1)
...
def build_data(variables: List[Dict[str, Union[str, float, int]]], extractions: List[Dict[str, Union[str, float, int]]]) -> Tuple[List[float], int]:
    ...
    extraction_df_train = extractions[extractions.variable_id.isin(
        variable_df_train.variable_id)]
    X, _ = dfs_to_data(variables, extractions, encoder)
    X_train, y_train = dfs_to_data(
        variable_df_train, extraction_df_train, encoder)
    return X_train, y_train, X


def sort_variable(extraction_variable_df: pd.DataFrame):
    df = extraction_variable_df.melt(
...
def get_confidence(variables: List[Dict[str, Union[str, float, int]]], extractions: List[Dict[str, Union[str, float, int]]]) -> List[float]:
    ...
    for variable_type in set(variables.variable_name.values):
        ...
        local_extractions = extractions[extractions.variable_id.isin(
            local_variables.variable_id)]
