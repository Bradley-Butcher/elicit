...
class LabelModel():
    ...
    def calculate_cov_OS(self):
        ...
        c = 1 / self.cov_S * \
            (1 + torch.mm(torch.mm(self.z.T, self.cov_O), self.z))
    ...
    def loss_prior_knowledge_probs(self, probs):
        ...
        return self.penalty_strength * torch.norm(
            (
                torch.Tensor(self.ground_truth_labels) - probs[:, 0]
            )[mask]
        ) ** 2
    ...
    def get_psi(self, label_matrix=None, cliques=None, nr_wl=None):
        ...
        # label_matrix = (label_matrix > 0).astype(int)
        ...
        for i in range(nr_wl):
            wl = label_matrix[:, i]
            wl_onehot = np.array([wl[:, None], 1 - wl[:, None]]).squeeze().T
            psi_list.append(wl_onehot)
            wl_idx[str(i)] = list(
                range(col_counter, col_counter + wl_onehot.shape[1]))
    ...
    def init_label_model(self, label_matrix, cliques, class_balance):
        ...
        self.y_set = np.array([0, 1])  # array of classes
    ...
    def predict(self, label_matrix=None, mu=None, P_Y=None, assign_train_labels=False):
        ...
        psi, _ = self.get_psi(label_matrix=label_matrix,
                              cliques=self.cliques, nr_wl=self.nr_wl)
        ...
        for i, clique in enumerate(self.cliques):
            clique_sums[:, i] = torch.Tensor(
                label_matrix[:, clique] != - 1).sum(dim=1) > 0
            ...
            if nr_non_abstain < self.nr_wl:
                if nr_non_abstain == 0:
                    ...
                    match_rows = np.where(
                        (
                            lambda_combs[:,
                                         cols_not_abstain[rows_not_abstain == i]]
        ...
        P_Y_given_lambda = (
            P_joint_lambda_Y[:, None] / self.P_lambda).clamp(0, 1)

        prob_labels = torch.cat(
            [1 - P_Y_given_lambda, P_Y_given_lambda], axis=1)
    ...
    def get_true_cov_OS(self, y_true):
        ...
        y_onehot = ((y_true[..., None] == self.y_set)
                    * 1).reshape((self.N, self.y_dim))
